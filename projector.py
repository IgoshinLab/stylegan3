...
def project(
    ...
    *,
    ...
    np.savez(f'{outdir}/vgg16_features.npz', target_features=target_features.unsqueeze(0).cpu().numpy(), synth_features=synth_features.detach().cpu().numpy())
...
python projector.py --outdir=out --network=/mnt/data/feature_extraction/featmodels/stylegan3/training-runs/00007-stylegan2-myxo-256x256-gpus1-batch32-gamma10/network-snapshot-000800.pkl --target /mnt/data/feature_extraction/myxo/data/Sorted_merged_crop/Core\ Wt\|\|DK1622\ 011912_560.tif
    """
    np.random.seed(seed)
    ...
    synth_image = (synth_image + 1) * (255/2)
    synth_image = synth_image.permute(0, 2, 3, 1).clamp(0, 255).to(torch.uint8)[0].cpu().numpy()
    if synth_image.ndim == 3 and img_color == "L":
        PIL.Image.fromarray(synth_image[:, :, 0], img_color).save(f'{outdir}/proj.png')
    else:
        PIL.Image.fromarray(synth_image, img_color).save(f'{outdir}/proj.png')
